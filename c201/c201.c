...
void List_Dispose( List *list ) {
	// First check if list has elements to dispose of
	if(list->firstElement != NULL) {
		// Elems have to be disposed of one by one
		ListElementPtr proc = list->firstElement;
		while(proc != NULL) {
			ListElementPtr next = proc->nextElement;
			free(proc);
			proc = next;
		}
	}
...
void List_InsertFirst( List *list, int data ) {
	...
	// In case of an empty list, the next elem will also be empty
	if(list->firstElement == NULL) {
		newElement->nextElement = NULL;
	} 
	// The list wasn't empty, so the new elem will point to the previous firt elem
	else {
		newElement->nextElement = list->firstElement;
	}

	newElement->data = data;
	// Set the newly created element as the first elem of the list
...
void List_GetFirst( List *list, int *dataPtr ) {
	...
	// Set dataPtr to data of first list element
...
void List_DeleteFirst( List *list ) {
	...
	if(list->firstElement != NULL) {
		// Get the second elem of the list
		ListElementPtr newFirstElem = list->firstElement->nextElement;

		// if the first elem is active, cancel its activity
		if(list->activeElement == list->firstElement) {
			list->activeElement = NULL;
		}
		// Free the memory of the first list elem
		free(list->firstElement);
		// Set a new first elem
...
void List_DeleteAfter( List *list ) {
	...
	if(list->activeElement != NULL && list->activeElement->nextElement != NULL ) {
		ListElementPtr elemPtr = list->activeElement->nextElement; 	// Set a pointer to the element we want to delete
		list->activeElement->nextElement = elemPtr->nextElement; 	// Set nextElement to the elem after the one we want to delete
		free(elemPtr); 												// we can now free the original elem that comes after the active one
	}
}
...
void List_InsertAfter( List *list, int data ) {
	...
	if(list->activeElement != NULL) { 								 // Initial condition check; ignore inactive list
		ListElementPtr newElem = malloc(sizeof(struct ListElement)); // Allocate memory for the new element
		if(newElem == NULL) { 										 // Malloc failed
			List_Error();
			return;
		}
		newElem->data = data; 									 	//Assign value to new elem
		newElem->nextElement = list->activeElement->nextElement; 	// Set elem after the new elem to the elem that comes after the current active elem
		list->activeElement->nextElement = newElem;				 	// Insert the new elem after the active one
...
void List_GetValue( List *list, int *dataPtr ) {
	if(list->activeElement != NULL) {			// Initial condition check
		*dataPtr = list->activeElement->data;	// Set dataPtr to current active elem data
	}
	else {
		List_Error();
		return;
...
void List_SetValue( List *list, int data ) {
	...
	if(list->activeElement != NULL) {		// Ignore non-active lists
		list->activeElement->data = data;	// Overwrite active elem data with desired value
	}
}
...
void List_Next( List *list ) {
	...
	if(list->activeElement != NULL) {							// ignore non-active lists
		list->activeElement = list->activeElement->nextElement;	// set the next elem as the active one
	}
}
